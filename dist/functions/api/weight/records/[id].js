...
export async function onRequest(context) {
  ...
  try {
    ...
    switch (request.method) {
      case 'GET':
        return await controller.getRecord(id, user, origin)

      case 'PUT':
        const updateData = await request.json()
        return await controller.updateRecord(id, updateData, user, origin)

      case 'DELETE':
        return await controller.deleteRecord(id, user, origin)

      default:
        return ApiResponse.error('不支持的请求方法', origin, 405)
    }
  } catch (error) {
    console.error('Weight Record [id] API error:', error)
