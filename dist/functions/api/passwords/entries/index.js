...
export async function onRequest(context) {
  ...
  try {
    ...
    switch (request.method) {
      case 'GET':
        return await controller.getEntries(user, origin, queryParams)

      case 'POST':
        const createData = await request.json()
        return await controller.createEntry(createData, user, origin)

      default:
        return ApiResponse.error('不支持的请求方法', origin, 405)
    }
  } catch (error) {
    console.error('Password entries API error:', error)
